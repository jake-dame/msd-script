/**
 * \file exec.cpp
 * \brief Exec and piping functions definitions for test_msdscript.
...
ExecResult exec_program(int argc, const char * const * argv, std::string input)
{
    ...
    else if ( pid == 0 )
    {
        ...
        // Getting here means that the execv failed
        {
...
//        std::cout << "COMMAND[0]: " << command[0] << " , COMMAND[1]: " << command[1] << std::endl; // TODO remove
